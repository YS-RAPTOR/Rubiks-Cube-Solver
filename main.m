...
centerDistance = averageLength * 1.4;

for i = 1:3
    for j=1:3
        index = (i-1)*3 + j;
        topLeft = [corners(1) + (j-1)*centerDistance, corners(2) + (i-1)*centerDistance];
        found = false;
        for k = 1:size(boundingBoxes, 1)
            if boundingBoxes(k,1) - topLeft(1) < averageLength && boundingBoxes(k,2) - topLeft(2) < averageLength && boundingBoxes(k,1) - topLeft(1) > -0.1 && boundingBoxes(k,2) - topLeft(2) > -0.1
        ...
        if ~found
            facelets(index, :) = [topLeft(1), topLeft(2), averageLength, averageLength];
        end
    end
end
end
